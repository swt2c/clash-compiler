...
vectorToCVectorList :: forall n. (KnownNat n, 1 <= n) => Vector n -> [CVector]
...
cvectorListToVector :: forall n. (KnownNat n, 1 <= n) => [CVector] -> Vector n
cvectorListToVector =
...
vectorToBitVector vec =
 ...
 where
  ...
  go ix s = replaceBit (maxBound - ix) (scalarToBit s)
