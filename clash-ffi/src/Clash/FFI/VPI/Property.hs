...
data UndefinedProperty a
  = UndefinedProperty (Property a) Handle
  deriving stock (Show)
...
getPropertyWith f err prop hdl = do
  ...
  Monad.when (value == err) $
    Sim.throw (UndefinedProperty prop hdl)
...
unsafeReceiveProperty
  :: (UnsafeReceive a, PropertyType (Received a))
  => Property (Received a)
  -> Handle
  -> SimCont o a
...
receiveProperty
  :: (Receive a, PropertyType (Received a))
  => Property (Received a)
  -> Handle
  -> SimCont o a
